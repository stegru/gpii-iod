...
fluid.registerNamespace("gpii.packageFile");
...
gpii.packageFile.fileIdentity = "gpii-iod-package-v1\0";

// file-id + 3x 32bit lengths (for packageData, signature, and installation file)
gpii.packageFile.headerLength = gpii.packageFile.fileIdentity.length + 3 * 4;
 ...
 */
gpii.packageFile.hashFile = function (file, algorithm) {
 ...
 */
gpii.packageFile.preparePackageData = function (packageData, installerFile) {
    ...
    if (installerFile) {
        ...
        hashPromise = gpii.packageFile.hashFile(installerFile);
    } else {
    ...
    hashPromise.then(function (installerHash) {
        ...
        if (!packageData.installerFilename) {
            computedPackageData.installerFilename = installerFile ? path.basename(installerFile) : undefined;
        }
 ...
 */
gpii.packageFile.readPEM = function (pem) {
 ...
 */
gpii.packageFile.signPackageData = function (packageData, key) {
    var togo = {};
    var data = fluid.copy(packageData);

    // Include the public key used to sign this package.
    data.publicKey = gpii.packageFile.readPEM(key.publicKey).toString("base64");
 ...
 *
 * @param {StringZPackageData} packageDataFile File (or an object) containing the package meta data
 * @param {Key} key The key used to sign the package data.
 * @param {String} saveAs The path of the new package file.
 * @return {Promise} Resolves when complete.
 */
gpii.packageFile.create = function (packageDataFile, key, saveAs) {
    ...
    var installerFile = origPackageData.$installerSource;

    var packageDataPromise = gpii.packageFile.preparePackageData(origPackageData, installerFile);

    packageDataPromise.then(function (packageData) {
        var signedPackageData = gpii.packageFile.signPackageData(packageData, key);

        // Initial file buffer contains the header, package data, and signature.
        var packageBuffer = Buffer.alloc(
            gpii.packageFile.headerLength + signedPackageData.buffer.length + signedPackageData.signature.length);

        // Create the header
        var offset = 0;
        offset += packageBuffer.write(gpii.packageFile.fileIdentity, "ascii");
 ...
 * @param {Boolean} options.keepOpen Keep the file open, returning the file descriptor in `PackageFileInfo.fd`. The
 *   position will be at the beginning of the installer payload.
 * @return {Promise<PackageFileInfo>} A promise resolving when the file has been read and parsed.
 */
gpii.packageFile.read = function (packageFile, options) {
    if (!options) {
        options = {};
    }
    var promise = fluid.promise();
    ...
    open(packageFile, "r").then(function (fd) {
        ...
        var work = [
            // Read the header
            gpii.packageFile.readHeader,
            // Get the package data and its signature
            gpii.packageFile.readPackageData,
            // Check the data and signature
            gpii.packageFile.verifyPackageData
        ];
 ...
 */
gpii.packageFile.readHeader = function (packageFileInfo) {
    var promise = fluid.promise();
    var buffer = Buffer.alloc(gpii.packageFile.headerLength);
    ...
    fs.read(packageFileInfo.fd, buffer, 0, buffer.length, null, function (err, bytesRead) {
        if (err) {
            ...
            header.identity = buffer.toString("ascii", offset, gpii.packageFile.fileIdentity.length);
            if (header.identity === gpii.packageFile.fileIdentity) {
 ...
 */
gpii.packageFile.readPackageData = function (packageFileInfo) {
    var promise = fluid.promise();
 ...
 */
gpii.packageFile.verifyPackageData = function (packageFileInfo) {
    var promise = fluid.promise();
